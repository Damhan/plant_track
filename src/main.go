...
func createTables(db *sql.DB) {
  _, err := db.Exec("CREATE TABLE IF NOT EXISTS vegPlants(ID TEXT, Plant TEXT, Age INTEGER)")
  checkErr(err)
}
...
func table_insertion(db *sql.DB, table string, insert_info string) {
  ...
  stmt, err:= db.Prepare("INSERT INTO vegPlants(ID,Plant,Age) VALUES(?,?,?)")
  checkErr(err)
...
func tableViewing(db *sql.DB,table_choice string) {
  rows, err := db.Query("SELECT * FROM vegPlants")
  checkErr(err)
  ...
  fmt.Println("Plant ID | Plant Name | Plant Veg Age")

  for rows.Next() {
    err = rows.Scan(&ID,&Plant,&Age)
    checkErr(err)
...
func main() {
  ...
  for {
    ...
    if strings.Compare(choice,"i") == 0{
      ...
      fmt.Println("Please insert in the form; 'id/plant/age' followed by an enter ")
